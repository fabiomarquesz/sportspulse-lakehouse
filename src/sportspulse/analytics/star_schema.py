...
from sportspulse.analytics.trimp import compute_banister_trimp, compute_edwards_trimp, estimate_calories_burned
...
def build_star_schema(
    ...
    dim_phase = pl.DataFrame({
        "phase_category": ["Rest", "Warmup", "Main_Workout", "Technical_Skill", "Resistance_Exercise", "High_Intensity_Interval", "Active_Recovery", "Recovery", "Unknown"],
        "phase_description": [
    ...
    session_keys = (
        df_silver_telemetry
        .select(["sport_code", "subject_id", "session_id"])
        .unique()
    ...
    fact_telemetry_1s = df_silver_telemetry.select([
        pl.concat_str([pl.col("sport_code"), pl.lit("_"), pl.col("subject_id"), pl.lit("_"), pl.col("session_id")]).alias("session_key"),
        pl.concat_str([pl.col("sport_code"), pl.lit("_"), pl.col("subject_id")]).alias("athlete_key"),
        pl.col("sport_code"),
        pl.col("subject_id"),
        pl.col("session_id"),
