    ...
    def clean_telemetry(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        return (
            df.with_columns(
                [
                    # Validity flags
                    ((pl.col("hr_bpm") >= self.hr_min) & (pl.col("hr_bpm") <= self.hr_max)).alias("is_hr_valid"),
                    ((pl.col("br_rpm") >= self.br_min) & (pl.col("br_rpm") <= self.br_max)).alias("is_br_valid"),
                    ((pl.col("rr_ms") >= self.rr_min) & (pl.col("rr_ms") <= self.rr_max)).alias("is_rr_valid"),
                ]
            )
            .with_columns(
                [
