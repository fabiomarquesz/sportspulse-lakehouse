...
def run_silver_pipeline(
    ...
    console.print("[bold cyan]🥈 Starting Silver Processing Pipeline[/bold cyan]")
    ...
    df_enriched = df_aligned.join(df_demographics, on=["sport_code", "subject_id", "session_id"], how="left").join(
        df_sports_meta, on="sport_code", how="left"
    )
    ...
    session_hr_rest = (
        df_enriched.filter(pl.col("phase_category") == "Rest")
        .group_by(["sport_code", "subject_id", "session_id"])
        .agg(pl.col("hr_bpm_clean").quantile(0.1).alias("hr_rest_est"))
    )

    df_enriched = (
        df_enriched.join(session_hr_rest, on=["sport_code", "subject_id", "session_id"], how="left")
        .with_columns(
            [
            ...
            [
                ...
                (
                    (
                        ...
                        / (pl.col("hr_max_est") - pl.col("hr_rest_est")).clip(lower_bound=10.0)
                    )
                    * 100.0
                )
                .clip(lower_bound=0.0, upper_bound=100.0)
                .alias("hr_reserve_pct"),
    ...
    console.print(
        f"  ✓ [green]Enriched Telemetry (Silver)[/green]: {len(df_enriched):,} rows -> [yellow]{silver_telemetry_file}[/yellow]"
    )
    ...
    console.print(
        f"  ✓ [green]Athletes Master (Silver)[/green]: {len(df_athletes)} athletes -> [yellow]{silver_athletes_file}[/yellow]"
    )
